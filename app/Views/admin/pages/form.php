                    ...
                    <div class="mb-3">
                        <div class="form-check form-switch">
                            ...
                            <label class="form-check-label" for="is_active">
                                ...
                                <span id="status_indicator" class="badge ms-2 <?= old('is_active', $page['is_active'] ?? 1) ? 'bg-success' : 'bg-secondary' ?>">
                                    <?= old('is_active', $page['is_active'] ?? 1) ? 'Published' : 'Draft' ?>
                                </span>
                            </label>
                        </div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3" id="header_order_group" style="display: <?= old('show_in_header', $page['show_in_header'] ?? 0) ? 'block' : 'none' ?>">
                        ...
                        <input type="number" class="form-control <?= isset($validation) && $validation->hasError('header_order') ? 'is-invalid' : '' ?>"
                            id="header_order" name="header_order" value="<?= old('header_order', $page['header_order'] ?? 0) ?>" min="0">
                        <?php if (isset($validation) && $validation->hasError('header_order')): ?>
                            <div class="invalid-feedback"><?= $validation->getError('header_order') ?></div>
                        <?php endif; ?>
                    ...
                    <div class="mb-3" id="footer_order_group" style="display: <?= old('show_in_footer', $page['show_in_footer'] ?? 0) ? 'block' : 'none' ?>">
                        ...
                        <input type="number" class="form-control <?= isset($validation) && $validation->hasError('footer_order') ? 'is-invalid' : '' ?>"
                            id="footer_order" name="footer_order" value="<?= old('footer_order', $page['footer_order'] ?? 0) ?>" min="0">
                        <?php if (isset($validation) && $validation->hasError('footer_order')): ?>
                            <div class="invalid-feedback"><?= $validation->getError('footer_order') ?></div>
                        <?php endif; ?>
                        <div class="form-text">Lower numbers appear first</div>
                    </div>
                </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateStatusIndicator() {
            if (isActiveCheckbox.checked) {
                statusIndicator.className = 'badge ms-2 bg-success';
                statusIndicator.textContent = 'Published';
            } else {
                statusIndicator.className = 'badge ms-2 bg-secondary';
        ...
        function updateHeaderOrderVisibility() {
            if (showInHeaderCheckbox.checked) {
                headerOrderGroup.style.display = 'block';
                headerOrderField.disabled = false;
            } else {
                headerOrderGroup.style.display = 'none';
                headerOrderField.value = '0'; // Reset to 0 when hidden
                headerOrderField.disabled = false; // Keep enabled so value is submitted
            }
        }

        function updateFooterOrderVisibility() {
            if (showInFooterCheckbox.checked) {
                footerOrderGroup.style.display = 'block';
                footerOrderField.disabled = false;
            } else {
                footerOrderGroup.style.display = 'none';
                footerOrderField.value = '0'; // Reset to 0 when hidden
                footerOrderField.disabled = false; // Keep enabled so value is submitted
            }
        }

        showInHeaderCheckbox.addEventListener('change', updateHeaderOrderVisibility);
        showInFooterCheckbox.addEventListener('change', updateFooterOrderVisibility);

        // Initialize the state based on current checkbox values
        updateHeaderOrderVisibility();
        updateFooterOrderVisibility();
    });
</script>
